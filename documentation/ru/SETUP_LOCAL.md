...
Если файла нет, будут использованы значения по умолчанию:
...
```bash
# Запуск всех контейнеров (backend, nginx, certbot)
docker-compose up
...
| URL | Описание |
|-----|---------|
| `http://localhost` | Главная страница (Home) |
| `http://localhost/poems` | Страница со всеми стихами |
| `http://localhost/about` | Страница "Об авторе" |
| `http://localhost/admin` | Администраторская панель |
| `http://localhost/api/health` | Проверка здоровья API |
...
БД автоматически инициализируется при первом запуске в файле `/backend/database.py`.
